...
- `GET /api/images/{id}` — metadata: size, human size, mime, width/height (via `getimagesize`), created/modified. Note `created` uses `ctime` (inode change time) since PHP has no portable file-birth-time API on Linux — it's a best-effort proxy, not a true creation date. Still implemented and routable, but the front end no longer calls it (see the detail-view note below) — kept for compatibility/direct API use.
...
**Detail-view metadata is computed entirely client-side from the raw image bytes** using [ExifReader](https://www.jsdelivr.com/package/npm/exifreader) (loaded via CDN in `index.html`, global `ExifReader`) — `showDetail()` no longer calls `/api/images/{id}` at all. It does one `fetch()` of `/api/images/{id}/raw`, and that single ArrayBuffer feeds both the display (wrapped in `URL.createObjectURL()` for the `<img src>`, revoked on the next detail open or window close via `revokeDetailObjectUrl()`) and `ExifReader.load(buffer)` for dimensions/camera/date-taken — no second network round trip for metadata. `size`/`modified`/`created` come straight from the already-loaded `allImages` entry (the bulk listing's `toSummary()` now includes `created` for exactly this reason), and mime type is guessed from the file extension via a small client-side `MIME_TYPES` map instead of `finfo`. Two ExifReader gotchas worth knowing:
