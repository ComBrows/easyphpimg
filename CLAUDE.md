...
- `GET /api/images/file/{filename}` — `toSummary()` fields (id/name/extension/size/created/modified) plus `size_human`/`mime`, and the file's contents inline as `base64`. Looked up by exact filename match against the cache (`Cache::findByName`), not by id. Loads the whole file into memory to encode it, so it's meant for occasional direct lookups, not bulk use.

There used to be a `GET /api/images/{id}` metadata endpoint (`ImagesController::show()` / `toDetail()`), dropped entirely — it called `getimagesize()` per request, and a real-world 10MB+ file with an unusual or malformed header could balloon PHP's memory use enough to 500. Dimensions/camera/date-taken are now exclusively a client-side ExifReader job (see below); no endpoint anywhere calls `getimagesize()` today. If you're tempted to re-add per-image dimension metadata to any endpoint, don't reach for `getimagesize()` — that's the exact failure mode this removal fixed.
...
- With enough metadata rows (e.g. a JPEG with Camera + Date taken on top of the usual five) and a short enough window, the `columns` layout can overflow *horizontally* instead of wrapping — CSS multi-column adds columns past the container width rather than scrolling vertically when content doesn't fit the fixed height. Pre-existing tradeoff of the columns-based layout, not something touched by the metadata work below; a taller `gravity` split would need to trade against the 80/20 image/metadata ratio.
...
**Detail-view metadata is computed entirely client-side from the raw image bytes** using [ExifReader](https://www.jsdelivr.com/package/npm/exifreader) (loaded via CDN in `index.html`, global `ExifReader`) — there's no server metadata endpoint left to call (see above). `showDetail()` does one `fetch()` of `/api/images/{id}/raw`, and that single ArrayBuffer feeds both the display (wrapped in `URL.createObjectURL()` for the `<img src>`, revoked on the next detail open or window close via `revokeDetailObjectUrl()`) and `ExifReader.load(buffer)` for dimensions/camera/date-taken — no second network round trip for metadata. `size`/`modified`/`created` come straight from the already-loaded `allImages` entry (the bulk listing's `toSummary()` now includes `created` for exactly this reason), and mime type is guessed from the file extension via a small client-side `MIME_TYPES` map instead of `finfo`. Two ExifReader gotchas worth knowing:
- Tag keys in the flat (non-`expanded`) mode returned by `ExifReader.load()` have **spaces in them** — `tags["Image Width"]`, not `tags.ImageWidth`. Verified directly against v4.41.0's actual output since the library's own docs are inconsistent on this.
- It understands JPEG/PNG/WebP/HEIC/TIFF file-format structure, not just EXIF proper, so `Image Width`/`Image Height` come back for plain PNGs with zero EXIF too (from the IHDR chunk) — but `ExifReader.load()` throws for anything it can't parse at all, so it's wrapped in try/catch with `width`/`height`/`camera`/`dateTaken` left `null` on failure; the rest of the metadata (size/type/created/modified) still renders from local data regardless.
